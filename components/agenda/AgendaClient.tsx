...
import { Accordion, AccordionContent, AccordionItem, AccordionTrigger } from "@/components/ui/accordion"; // Not used, wait, user wants horizontally scrolling columns, not accordions!
...
export default function AgendaClient({ events }: Props) {
  ...
  const columns = useMemo(() => {
    ...
    for (let i = 0; i < totalMonths; i++) {
      ...
      monthEvents.sort((a, b) => new Date(a.dateEtHeure).getTime() - new Date(b.dateEtHeure).getTime());
  ...
  if (events.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center p-12 text-center text-foreground-secondary h-[400px]">
        <h3 className="text-2xl font-bold font-(family-name:--font-dynaPuff) mb-2">Agenda</h3>
  ...
  return (
    <div className="w-full flex-grow flex flex-col p-4 md:p-8 overflow-hidden relative z-10 pointer-events-auto">
      <h2 className="text-5xl font-bold mb-8 text-center" style={{ fontFamily: "var(--font-dynaPuff)", color: "var(--foreground)" }}>
        Agenda
      </h2>
          ...
          {columns.map((col, i) => (
            ...
            >
              <div 
                ...
                style={{
                  color: col.events.length > 0 ? "var(--primary)" : "var(--foreground-secondary)",
                  writingMode: col.events.length > 0 ? "horizontal-tb" : "vertical-lr",
